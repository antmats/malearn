...
def create_decision_stump():
    return Tree().split_node(index=0)
...
def create_decision_tree(depth):
    ...
    while (
        tree.depth < depth 
        or len(tree.inner_nodes) < 2 ** depth - 1
    ):
...
class Tree:
    def __init__(self, root_index=0):
        self.nodes = {root_index: Node(index=root_index)}
        self.depth = 0

    def split_node(self, index):
        if index not in self.nodes:
            raise ValueError(f"Node with index {index} does not exist.")
        
        node = self.nodes[index]
        
        if node.left_child is not None or node.right_child is not None:
            raise ValueError(f"Node with index {index} already has children.")
        
        node.add_children()
        
        self.nodes[node.left_child.index] = node.left_child
        self.nodes[node.right_child.index] = node.right_child
    ...
    def unsplit_node(self, index):
        ...
        if node.left_child is None and node.right_child is None:
            raise ValueError(f"Node with index {index} has no children.")
        
        if node.left_child:
            del self.nodes[node.left_child.index]
        if node.right_child:
            del self.nodes[node.right_child.index]
    ...
    def _update_depth(self):
        if not self.nodes:
            ...
            self.depth = max(node.layer_index for node in self.nodes.values())
    
    def get_inner_node_index(self, node):
        """Get the inner node index of a given node."""
        if not isinstance(node, Node):
            raise ValueError("Input must be a `Node` object.")
        if node.is_leaf:
            raise ValueError(f"Node with index {node.index} is not an inner node.")
        
        for i, inner_node in enumerate(self.inner_nodes.values()):
            if (inner_node.index == node.index) and (inner_node is node):
                return i
        
        raise ValueError(f"Node with index {node.index} is not an inner node.")
    
    def get_leaf_node_index(self, node):
        """Get the leaf node index of a given node."""
        if not isinstance(node, Node):
            raise ValueError("Input must be a `Node` object.")
        if not node.is_leaf:
            raise ValueError(f"Node with index {node.index} is not a leaf node.")
        
        for i, leaf_node in enumerate(self.leaf_nodes.values()):
            if (leaf_node.index == node.index) and (leaf_node is node):
                return i
        
        raise ValueError(f"Node with index {node.index} is not a leaf node.")

    def get_inner_nodes_at_layer(self, layer_index):
        return {
            index: node for index, node in self.inner_nodes.items()
    ...
    def inner_nodes(self):
        inner_nodes = {
            index: node for index, node in self.nodes.items()
            if not node.is_leaf
        }
        return dict(sorted(inner_nodes.items()))
    ...
    def leaf_nodes(self):
        ...
        return dict(sorted(leaf_nodes.items()))
...
def get_node_indices_per_layer(depth):
    ...
    for i_layer in range(depth):
        node_indices_per_layer[i_layer] = list(range(i_start, i_stop))
        i_start = i_stop
