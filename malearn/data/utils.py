...
def get_random_generators(seed=None):
...
def pick_coeffs(X, idxs_obs=None, idxs_nas=None, self_mask=False, g=None):
    n, d = X.shape
    if self_mask:
        coeffs = torch.randn(d, generator=g)
        Wx = X * coeffs
        coeffs /= torch.std(Wx, 0)
    else:
        d_obs = len(idxs_obs)
        d_na = len(idxs_nas)
        coeffs = torch.randn(d_obs, d_na, generator=g)
...
def MAR_mask(X, p, p_obs, seed=None):
    ...
    """
    r, g = get_random_generators(seed)
    ...
    idxs_obs = r.choice(d, d_obs, replace=False)
    ...
    coeffs = pick_coeffs(X, idxs_obs, idxs_nas, g=g)
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X[:, idxs_obs], coeffs, p)

    ps = torch.sigmoid(X[:, idxs_obs].mm(coeffs) + intercepts)

    ber = torch.rand(n, d_na, generator=g)
    mask[:, idxs_nas] = ber < ps

    return mask


def MNAR_mask_logistic(X, p, p_params =.3, exclude_inputs=True, seed=None):
    """
    ...
    idxs_params = r.choice(d, d_params, replace=False) if exclude_inputs else np.arange(d)
    ...
    ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, idxs_params, idxs_nas, g=g)
    ### Pick the intercepts to have a desired amount of missing values
    ...
    if exclude_inputs:
        mask[:, idxs_params] = torch.rand(n, d_params, generator=g) < p

    return mask


def MNAR_self_mask_logistic(X, p, seed=None):
    """
    ...
    ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X, self_mask=True, g=g)
    ### Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X, coeffs, p, self_mask=True)

    ps = torch.sigmoid(X * coeffs + intercepts)

    ber = torch.rand(n, d, generator=g) if to_torch else r.rand(n, d)
    mask = ber < ps if to_torch else ber < ps.numpy()

    return mask


def MNAR_mask_quantiles(X, p, q, p_params, cut='both', MCAR=False, seed=None):
    """
    ...
    idxs_na = r.choice(d, d_na, replace=False) ### select at least one variable with missing values
    ...
    ## Add a mcar mecanism on top
        mask = mask | (torch.rand(n, d, generator=g) < p)
